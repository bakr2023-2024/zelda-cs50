...
function PlayState:init()
	...
	self.player.stateMachine = StateMachine({
		["walk"] = function()
			return PlayerWalkState(self.player, self.dungeon)
		end,
		["idle"] = function()
			return PlayerIdleState(self.player, self.dungeon)
		end,
		["swing-sword"] = function()
			return PlayerSwingSwordState(self.player, self.dungeon)
		end,
		["pot-lift"] = function()
			return PlayerLiftPotState(self.player, self.dungeon)
		end,
		["pot-walk"] = function()
			return PlayerWalkPotState(self.player, self.dungeon)
		end,
		["pot-idle"] = function()
			return PlayerIdlePotState(self.player, self.dungeon)
		end,
	})
	...
	if renderMap then
		...
		for y = 1, #self.dungeon.rooms do
			for x = 1, #self.dungeon.rooms[y] do
				...
				love.graphics.rectangle("fill", (x - 1) * 8 + VIRTUAL_WIDTH - 90, (y - 1) * 8 + 18, 8 - 2, 8 - 2)
				love.graphics.setColor(1, 1, 1, 0.5)
			end
		end
