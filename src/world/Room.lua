...
function Room:generateObjects()
	...
	-- generate N random pots
	local potCount = math.random(1, 4)
	for i = 1, potCount do
...
function Room:update(dt)
	...
	for i = #self.objects, 1, -1 do
		...
		if object.projectile and object.projectile.collided then
			table.remove(self.objects, i)
		end
	end
end
...
function Room:render()
	...
	love.graphics.stencil(function()
		...
		love.graphics.rectangle(
			"fill",
			MAP_RENDER_OFFSET_X + (MAP_WIDTH * TILE_SIZE),
			MAP_RENDER_OFFSET_Y + (MAP_HEIGHT / 2) * TILE_SIZE - TILE_SIZE,
			TILE_SIZE * 2 + 6,
			TILE_SIZE * 2
		)
		...
		love.graphics.rectangle(
			"fill",
			MAP_RENDER_OFFSET_X + (MAP_WIDTH / 2) * TILE_SIZE - TILE_SIZE,
			-TILE_SIZE - 6,
			TILE_SIZE * 2,
			TILE_SIZE * 2 + 12
		)
